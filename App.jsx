...
const CITIES = [
  { id: 'india',    city: 'Mumbai',    country: 'India',          flag: '🇮🇳',     tz: 'Asia/Kolkata',      coords: [72.88,   19.08] },
  { id: 'chicago',  city: 'Chicago',   country: 'United States',  flag: '🇺🇸',     tz: 'America/Chicago',   coords: [-87.63,  41.88] },
  { id: 'scotland', city: 'Edinburgh', country: 'Scotland',       flag: '🏴󠁧󠁢󠁳󠁣󠁴󠁿', tz: 'Europe/London',     coords: [-3.19,   55.95] },
  { id: 'portugal', city: 'Lisbon',    country: 'Portugal',       flag: '🇵🇹',     tz: 'Europe/Lisbon',     coords: [-9.14,   38.72] },
  { id: 'manila',   city: 'Manila',    country: 'Philippines',    flag: '🇵🇭',     tz: 'Asia/Manila',       coords: [120.98,  14.60] },
  { id: 'boston',   city: 'Boston',    country: 'United States',  flag: '🇺🇸',     tz: 'America/New_York',  coords: [-71.06,  42.36] },
]

function getTime(tz) {
  return new Date().toLocaleTimeString('en-US', { timeZone: tz, hour12: true, hour: '2-digit', minute: '2-digit', second: '2-digit' })
}
...
function getHMS(tz) {
  const s = new Date().toLocaleTimeString('en-US', { timeZone: tz, hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' })
...
function ClockCard({ city, delay }) {
  ...
  return (
    ...
    >
      ...
      <span style={{ fontFamily: "'DM Serif Display', serif", fontSize: '1.1rem', display: 'block', marginBottom: '0.1rem', color: 'var(--text)' }}>{city.city}</span>
      <span style={{ fontSize: '0.58rem', letterSpacing: '0.18em', color: 'var(--muted)', textTransform: 'uppercase', display: 'block', marginBottom: '0.9rem' }}>{city.country}</span>
      <span style={{ fontFamily: "'DM Mono', monospace", fontSize: '1.55rem', fontWeight: 500, color: 'var(--accent)', letterSpacing: '-0.02em', display: 'block', lineHeight: 1 }}>{time}</span>
      <span style={{ fontSize: '0.6rem', color: 'var(--muted)', letterSpacing: '0.1em', marginTop: '0.4rem', display: 'block' }}>{date}</span>
      <span style={{ position: 'absolute', bottom: '0.9rem', right: '1.1rem', fontSize: '0.52rem', letterSpacing: '0.15em', color: 'rgba(100,160,220,0.2)', textTransform: 'uppercase' }}>
  ...
  return (
    <div style={{ position: 'relative', background: '#071524', borderRadius: '0 0 14px 14px' }}>
        ...
        >
          ...
          {CITIES.map(city => (
            ...
            >
              ...
              <circle r={3.5} fill="#c9a84c" stroke="#08111e" strokeWidth={1.2} style={{ cursor: 'pointer' }} />

              {tooltip === city.id && (
                <g transform="translate(0, -18)">
                  <rect x={-42} y={-26} width={84} height={30} rx={4}
                    fill="rgba(8,17,30,0.95)" stroke="rgba(100,160,220,0.2)" strokeWidth={0.6} />
      ...
      <div style={{
        ...
        fontSize: '0.52rem', color: 'rgba(100,160,220,0.3)',
...
export default function App() {
  ...
  return (
    <div style={{ position: 'relative', zIndex: 1, maxWidth: '1120px', margin: '0 auto', padding: '3rem 1.5rem 4rem' }}>
      ...
      {/* Header */}
      <header style={{ textAlign: 'center', marginBottom: '2.5rem', animation: 'fadeDown 0.8s ease both' }}>
        ...
        <h1 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 'clamp(2.2rem, 5.5vw, 3.8rem)', letterSpacing: '-0.02em', lineHeight: 1, color: 'var(--text)' }}>
      ...
      <footer style={{
        ...
        fontSize: '0.55rem', color: 'rgba(232,228,216,0.18)',
        letterSpacing: '0.18em', animation: 'fadeUp 1s 0.7s ease both',
      }}>
